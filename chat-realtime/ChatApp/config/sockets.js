...
module.exports.sockets = {
  ...
  onConnect: function(session, socket) {
	...
	// TODO. - Kiểm tra user đã authenticate với passport.
			// thêm object Zsocket
			// thêm một người mới nếu chưa authenticate trước đó
		if(session.passport){
			// user đã authenticate với passport
			if(session.passport.user){
				...
				var socket_new;
				if(user){
					// user đã tạo trước đó
					user.changeStatus(1); // user online;
				}else{
					// thêm mới một user
					user = new Zchat.Zuser(user_id);
					Zusers.push(user);
					isNew = true;
				}
				socket_new = new Zchat.Zsocket(socket, user);
				socket_new.updateFuncCode();
				
				console.log('function code:' + socket_new.func_code);
				console.log('code_1:' + socket_new.code_1);
				
				Zsockets.push(socket_new);
				user.sockets.push(socket_new);
				UserModel.findOne({ id: user.user_id }, function(err, puser) {
					if(err){
				...
				/* socket.disconnect(); 
				 * change: add socket to ZsocketsGuest
				*/
				var socket_new = new Zchat.Zsocket(socket, null);
				ZsocketsGuest.push(socket_new);
			}
		}else{
			// user chưa authenticate, tạo socket cho tài khoản guest.
			//var socket_new = new Zchat.Zsocket(socket,null);
			//Zsockets.push(socket_new);
			/* socket.disconnect();
			 * change: add socket to ZsocketsGuest
  ...
  onDisconnect: function(session, socket) {
	...
	// TODO. Tìm Zsocket trong mảng Zsockets để remove
			// remove Zuser nếu không còn socket nào
			// remove room nếu không còn user nào.
			ZchatUtils.removeSocket(socket);
	// TODO. 
  },
